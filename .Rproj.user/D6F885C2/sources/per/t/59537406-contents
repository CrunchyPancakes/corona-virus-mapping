...
prep <- data.frame(seq=character(0),ord=numeric(0),stringsAsFactors = F)

names <- data.frame(nam=character(0),stringsAsFactors = F)

for(fil in fils){
  set <- readDNAStringSet(fil)
  nam <- as.numeric(gsub("20VR","",set@ranges@NAMES))
  seq <- substr(toString(set),56,29797)
  prep <- rbind(prep,data.frame(seq=seq,ord=nam))
  names <- rbind(names,data.frame(nam=set@ranges@NAMES))
}

prep <- prep[order(prep$ord),]
names <- names[order(prep$ord),]

names <- as.character(names)

write_csv(as.data.frame(prep$seq),"ordered_nz.txt",col_names = F)
