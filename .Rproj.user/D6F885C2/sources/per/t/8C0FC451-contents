...
library(tesseract)
library(readr)
set <- readDNAStringSet("gisaid_cov2020_sequences.fasta")
...
ds <- data.frame(seq=seqs,dats=dats,stringsAsFactors = F)
ds <- ds[order(ds$dats),]
ds <- ds[which(nchar(ds$dats)==10),]

names <- names[order(ds$dats)]

# for(i in 1:nrow(ds)){
#   write_csv(as.data.frame(substr(toString(set[toString(ds$seq[i])]),56,29797)),"ordered.txt", col_names = F, append = T)
# }
